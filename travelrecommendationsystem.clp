...
;defining abstract structure template for places
...
(place
	...
	(weather Moderate)
	(budget under20k 20k-50k )
	(duration under5days 5-7days over7days)
)
...
;rules to get user inputs

(defrule travelrecommend(declare (salience 1))
 =>
(printout t crlf)
(printout t "--------------------------------------------------------------------------------------------" crlf)
(printout t "--------------------------------TRAVEL RECOMMENDATION SYSTEM--------------------------------" crlf)
(printout t "--------------------------------------------------------------------------------------------" crlf)
(printout t crlf) 
)
...
(printout t "Enter Weather-----------(Hot|Cold|Moderate|Rainy)" crlf )
	(assert (wt (read)))
(printout t "Enter Duration-----------(under5days|5-7days|over7days)" crlf )
	(assert (dt (read)))
